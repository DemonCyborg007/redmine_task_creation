...
# ============================================================================
REDMINE_URL = os.getenv("REDMINE_URL", "https://20.207.146.131/redmine").rstrip("/")
API_KEY = os.getenv("REDMINE_API_KEY", "0b783d90745974e0dc701ad1def0e0a0b2aeb9b9")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for tracker in trackers:
                if tracker.get("name").lower() == tracker_name.lower().strip():
                    return tracker["id"]
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for priority in priorities:
                if priority.get("name").lower() == priority_name.lower().strip():
                    return priority["id"]
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for version in versions:
                if version.get("name", "").lower() == vn.lower():
                    return version["id"]
...
def clean_subject(subject):
    """Clean subject by removing pipe characters and extra whitespace."""
    if not subject:
        return subject
    ...
    try:
        ...
        for row_num, row in enumerate(reader, 2):
            tasks.append({
                ...
                "target_version": row.get("Target version", "").strip(),
